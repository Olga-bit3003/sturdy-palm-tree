...
def get_win_sheck(fd, symbol):
    ...
    for i in range(3):
     if fd [1][i] == fd [0][i] == fd [2][i] == symbol:
        flag_win = True
    if fd[0][0] == fd[1][1] == [2][2] == symbol:
        flag_win = True 
    if fd[0][2] == fd[1][1] == [2][0] == symbol:
        flag_win = True 
    return flag_win
...
while mainIoop:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and not  game_over:
            ...
            player_win = get_win_sheck(field, "x")
            ai_win = get_win_sheck(field, 0)
            if player_win or ai_win:
                game_over = True
                if player_win:
                    pygame.display.set_caption("Вы победили!")
                else:
                    pygame.display.set_caption("Увы, Бот победил")
